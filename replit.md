...
- **CAO Selection & Dynamic AI Instructions (Oct 2025):** Tenant-wide CAO preference system allowing users to choose between 2 uitzend-CAO's: NBBU or ABU. Selection happens during signup and is stored in tenant profile. **DOCUMENT STRATEGY**: AI always uses "gekozen CAO + alle resterende documenten" - if ABU is chosen, AI uses ABU + alle andere documenten (NO NBBU); if NBBU is chosen, AI uses NBBU + alle andere documenten (NO ABU). **CRITICAL**: The AI may NEVER use both ABU and NBBU simultaneously - only the chosen CAO is used. System instructions in `cao_config.py` explicitly enforce: "ALTIJD gebruiken: {gekozen CAO} + ALLE resterende documenten" and "STRIKT VERBODEN: Gebruik NOOIT de {alternative_cao} CAO". Admins can change CAO preference in dashboard settings with organization-wide impact warnings. Default: NBBU for all legacy data.
...
- **CAO Selection Feature - Final Implementation (Oct 18, 2025):** Tenant-wide CAO preference system refined to 2 options (NBBU and ABU - both uitzend-CAO's). **Database**: Added `cao_preference` field (varchar 50, default 'NBBU') to both Tenant and PendingSignup models. **Signup Flow**: Integrated CAO selector dropdown in signup form (signup_tenant.html) with 2 options only. Selection saved to PendingSignup + Stripe metadata. **AI Document Strategy**: Created `cao_config.py` with dynamic system instruction generator that implements "gekozen CAO + resterende documenten" strategy. System instructions explicitly state: "ALTIJD gebruiken: {gekozen CAO} + ALLE resterende documenten (arbeidsrecht, detacheringsregels, andere CAO's BEHALVE {alternative_cao})" and "STRIKT VERBODEN: Gebruik NOOIT de {alternative_cao} CAO". If ABU chosen → ABU + alle andere documenten (NO NBBU). If NBBU chosen → NBBU + alle andere documenten (NO ABU). The AI may NEVER use both CAO's simultaneously. Chat endpoint uses `tenant.cao_preference` to apply this exclusion strategy. **Admin Dashboards**: Both tenant admin and super admin dashboards have CAO dropdown with only NBBU/ABU options. Route `/super-admin/tenants/<id>/cao` handles super admin CAO updates. **Validation**: `validate_cao_preference()` restricts to NBBU/ABU only. **UI Messaging**: All helper texts clarify "Lexi gebruikt altijd alle 1.000+ documenten" to prevent filtering assumptions.
