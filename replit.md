...
- **CAO Selection & Dynamic AI Instructions (Oct 2025):** Tenant-wide CAO preference system allowing users to select their applicable CAO (NBBU, ABU, Hortiplan, CAO Uitzendkrachten, Agrarisch, Overig). Selection happens during signup and is stored in tenant profile. AI responses dynamically adjust based on selected CAO via custom system instructions in `cao_config.py`. Admins can change CAO preference in dashboard settings with organization-wide impact warnings. Default: NBBU for all legacy data.
...
- **CAO Selection Feature (Oct 18, 2025):** Complete implementation of tenant-wide CAO preference system. **Database**: Added `cao_preference` field (varchar 50, default 'NBBU') to both Tenant and PendingSignup models with ALTER TABLE migrations for legacy data. **Signup Flow**: Integrated CAO selector dropdown in signup form (prijzen.html) with 6 options (NBBU, ABU, Hortiplan, CAO Uitzendkrachten, Agrarisch, Overig). Selection saved to PendingSignup + Stripe metadata. **Provisioning**: cao_preference correctly copied from PendingSignup to Tenant during account creation with safe fallback to 'NBBU'. **AI Integration**: Created `cao_config.py` with dynamic system instruction generator (`get_system_instruction()`) that customizes Lexi's responses based on selected CAO - all 6 CAO options fully supported with dedicated system instructions. Chat endpoint now uses `tenant.cao_preference` to apply CAO-specific instructions. **Hardcoded NBBU Fixed**: Removed hardcoded NBBU reference from `services.py` system_instruction - now uses generieke fallback die altijd wordt overschreven door cao_config.py. **Tenant Admin Dashboard**: Added CAO Settings card with validated dropdown, confirmation dialog, and organization-wide impact warning. **Super Admin Dashboard**: Added CAO kolom in tenant tabel met inline dropdown editor - super admins kunnen direct CAO voorkeur per tenant aanpassen. Route `/super-admin/tenants/<id>/cao` handles updates. **Bug Fix**: Expanded cao_config.py to support all 6 CAO options (was only NBBU/ABU initially, causing chat failures for other CAOs). Architect verdict: "PASS - CAO-voorkeur stroom werkt end-to-end, dynamische AI-instructies beperken antwoorden tot gekozen CAO, geen security issues."
