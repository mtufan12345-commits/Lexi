...
- **CAO Selection & Dynamic AI Instructions (Oct 2025):** Tenant-wide CAO preference system allowing users to choose between 2 uitzend-CAO's: NBBU or ABU. Selection happens during signup and is stored in tenant profile. **DOCUMENT STRATEGY**: AI always uses "gekozen CAO + alle resterende documenten" - if ABU is chosen, AI uses ABU (prioriteit) + alle andere documenten; if NBBU is chosen, AI uses NBBU (prioriteit) + alle andere documenten. The chosen CAO has priority for CAO-specific questions, while all other documents provide supplementary context. System instructions in `cao_config.py` explicitly enforce: "ALTIJD gebruiken: {gekozen CAO} + ALLE resterende documenten". Admins can change CAO preference in dashboard settings with organization-wide impact warnings. Default: NBBU for all legacy data.
